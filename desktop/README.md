...
Если после установки приложения на macOS вы видите ошибку:
> **«Приложение „OKAK“ повреждено, и его не удается открыть. Переместите приложение в Корзину.»**

Это стандартная реакция системы защиты Gatekeeper на приложения без цифровой подписи Apple Developer. Чтобы исправить это:

1. Откройте **Терминал**.
2. Выполните команду (потребуется ввести пароль администратора):
   ```bash
   sudo xattr -cr /Applications/OKAK.app
   ```
   *(Если приложение установлено не в `/Applications`, укажите правильный путь к файлу `.app`)*.
3. После этого приложение запустится в обычном режиме.
